/* 목 API 가 정한 대로 동작하는지 센다.  npm run api:verify
   ...
   여기 있는 것은 「되는 것」 확인이 아니라 **막는 쪽**과 **정한 것**이다.
     · ADR-0004 상태값 경계 — 한글로 필터해도 영문 enum 으로 필터해도 같은 결과가 나온다
     · ADR-0006 협력업체 목록 기본 정렬 — companyName(업체명순)
...
ok(names.length === 48 && JSON.stringify(names) === JSON.stringify([...names].sort((a, b) => a.localeCompare(b))),
  'ADR-0006 · 기본 정렬이 업체명순이다', names.slice(0, 2).join(' · ') + ' …')
...
ok(byLabel.length === 46, 'ADR-0004 · 한글 라벨로 필터하면 걸린다', byLabel.length + '곳')
ok(byCode.length === 46, 'ADR-0004 · 영문 enum 으로 필터해도 같은 결과다 (실서버가 보내는 값)', byCode.length + '곳')
ok(byLabel.every(s => s.tie === '협력유지중'), 'ADR-0004 · 응답의 상태값은 화면용 한글로 바뀌어 나온다')
ok(toLabel('QUALIFIED') === '적격' && toCode('미제출') === 'NOT_SUBMITTED', 'ADR-0004 · 매핑표 왕복')
ok(toLabel('접수 불가') === '접수 불가', 'ADR-0004 · 매핑에 없는 값은 지어내지 않고 그대로 통과한다')
