...
async function load() { rows.value = allRows(await Parts.list({ size: 1000 }), 'GET /parts'); loading.value = false }
onMounted(load)
/* 9번 — 부품명 검색과 협력업체(다중 가능)·CN코드 필터 */
const facets = [
...
<template>
  <ViewHead api="UC-02 · 부품 · GET /parts">
    ...
    <template #acts>
      <!-- 11번 엑셀 일괄 등록은 우선순위가 낮아 보류다 (docs/product/README.md). 버튼만 두고 하는 일을 숨기지 않는다 -->
  ...
  <div class="parts" ref="list" v-reveal>
    ...
    <SkeletonRows v-if="loading" :rows="6" :cols="['1fr', '190px', '130px', '240px']" />
    <!-- 10번 단일 조회 — 행을 누르면 부품 상세로 간다 -->
    <div v-for="p in t.filtered" :key="p.id" :data-flip="p.id" class="pt link" :class="{ gap: !p.factor }"
         v-clickable :aria-label="`${p.name} 상세`" @click="router.push(`/parts/${p.id}`)">
      <b>{{ p.name }}</b>
      <span class="sup">{{ p.cn }}</span>
      <span class="val">{{ p.supplier }}</span>
      <StatusChip v-if="p.factor" :label="`${p.factor} · ${p.unit}`" tone="complete" />
  ...
  <ActionBar :title="`부품 ${rows.length}개 중 벤치마크 미등록 ${gaps}개`" note="벤치마크 팩터는 평균값 비교(± 30%)의 기준이 됩니다. 비어 있으면 그 부품이 들어간 완제품은 신고할 수 없습니다">
    <button class="btn" @click="router.push('/products')">완제품에서 쓰기</button>
  </ActionBar>
  <PartForm :open="adding" @close="adding = false" @created="load" />
