...
import { PRODUCTS, EMISSIONS, INBOX, SUBMISSION, QUEUE, CHECKS, SEVERITIES, DEADLINES, REMINDERS, DISPATCH, TONES } from './fixtures'
...
export const Parts = {
  ...
  facets: () => request(() => ({ supplier: count(PRT, 'supplier'), cn: count(PRT, 'cnGroup'), total: PRT.length })),
...
export const Products = {
  ...
  facets: () => request(() => ({ cn: count(PRODUCTS, 'cnGroup'), total: PRODUCTS.length })),
  /** GET /products/{id}/emissions */
  emissions: id => request(() => EMISSIONS[id] || EMISSIONS['hr-2400']),
}

/* ── UC-04 이메일 접수 ──────────────────────────────────── */
export const Inbox = {
  /** GET /submissions/inbox */
  list: () => request(() => ({ items: INBOX })),
  /** PUT /submissions/{id}/supplier — 미확인 건에 협력업체를 직접 지정 */
  assign: (id, supplierName) => request(() => {
...
export const Analysis = {
  ...
  parse: (id, opts) => startTask('parse', { result: id, ...opts }),
  /** GET /tasks/{id} */
  task: getTask,
  /** GET /submissions/{id} */
  get: id => request(() => SUBMISSION),
}

/* ── UC-07 검토 ─────────────────────────────────────────── */
export const Review = {
  /** GET /submissions?status=review */
  queue: () => request(() => ({ items: QUEUE })),
  /** PUT /submissions/{id} → CONFIRMED */
  confirm: id => request(() => {
    if (SUBMISSION.missingFields.length)
      throw new ApiError(400, 'MISSING_FIELDS', '누락 항목이 있어 확정할 수 없습니다', SUBMISSION.missingFields)
    return { id, status: 'CONFIRMED', confirmedAt: '2026-09-02T15:10:00+09:00' }
  }),
  /** PUT /submissions/{id}/reject */
  reject: (id, reason) => request(() => ({ id, status: 'REJECTED', reason })),
}

/* ── UC-08 적격 판정 ────────────────────────────────────── */
export const Rules = {
  /** GET /rules */
  get: () => request(() => ({ checks: CHECKS, severities: SEVERITIES })),
  /** PUT /rules */
  update: body => request(() => ({ ...body, appliesFrom: 'next' })),
}

/* ── UC-09 마감 ─────────────────────────────────────────── */
export const Deadlines = {
  /** GET /deadlines */
  list: () => request(() => ({ items: DEADLINES })),
  /** GET /deadlines/current/unsubmitted */
  unsubmitted: () => request(() => ({ items: REMINDERS })),
  /** POST /reminders { supplierIds } */
  remind: ids => request(() => ({ sent: ids.length, sentAt: '2026-09-03' })),
}

/* ── UC-10·11 피드백 ────────────────────────────────────── */
export const Feedback = {
  /** POST /feedback/draft { submissionId, tone } */
  draft: (submissionId, tone = '격식') => request(() => ({ submissionId, tone, body: TONES[tone] })),
  /** GET /feedback */
  list: () => request(() => ({ items: DISPATCH })),
  /** PUT /feedback/{id}/confirm */
  confirm: id => request(() => ({ id, status: '발송 대기', locked: true })),
  /** POST /feedback/{id}/send */
  send: ids => request(() => ({ sent: ids.length })),
  /** POST /feedback/{id}/resend */
  resend: id => request(() => ({ id, resendCount: 2 })),
}
