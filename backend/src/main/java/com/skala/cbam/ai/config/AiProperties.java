...
/**
 * AI 호출 설정 (ADR-0013). 값은 {@code .env} 의 {@code AI_BASE_URL}·{@code AI_API_KEY}·
 * {@code AI_MODEL} 에서 온다 — {@code application.yml} 이 그 이름들을 참조한다.
 *
 * <p><b>키가 없어도 앱은 뜬다</b>(ADR-0013 ③). {@link #isConfigured()} 가 false 면 AI 호출이
 ...
 * @param timeout 읽기 타임아웃. 스트리밍이라 토큰이 계속 오는 동안은 갱신된다(ADR-0013 ②)
 */
