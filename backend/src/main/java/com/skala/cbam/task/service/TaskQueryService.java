...
import com.skala.cbam.task.service.port.UnregisteredPartCountProvider;
 ...
 *
 * <p>{@code Task} 엔티티는 {@code feedback} 패키지에 있다. 여기로 옮기지 않은 이유는 ADR-0011 ① —
 * {@code Task} 가 {@code Feedback} 을 참조하고 있어 옮겨도 의존 방향이 그대로 따라온다.
 *
 * <p><b>자원을 찾는 길이 둘이다</b>(ADR-0011 ②). 새 작업은 {@code task_resource} 에 기록하고,
 * CBAM-88 이 먼저 만든 발송·재생성 작업은 {@code task} 의 FK 를 갖는다. 이 서비스가 그 둘을
...
public class TaskQueryService {
    ...
    private final UnregisteredPartCountProvider unregisteredPartCountProvider;

    public TaskDetailResponse getDetail(String taskId) {
        Task task = taskRepository.findById(taskId)
                .orElseThrow(() -> new TaskException(TaskErrorCode.TASK_NOT_FOUND));

        List<TaskResource> recorded = taskResourceRepository.findByTaskIdOrderBySequenceNoAsc(taskId);

        TaskResourceType resourceType = recorded.isEmpty()
                ? fallbackResourceType(task)
                : recorded.get(0).getResourceType();

        List<Long> resourceIds = recorded.isEmpty()
                ? fallbackResourceIds(task)
                : recorded.stream().map(TaskResource::getResourceId).toList();
        ...
        int unregisteredPartCount = resourceType == TaskResourceType.SUBMISSION
                ? unregisteredPartCountProvider.countUnregisteredParts(resourceIds)
                : 0;
    ...
    /** CBAM-88 이 {@code task_resource} 없이 만든 작업 — 타입이 곧 자원 종류다. */
     ...
     * 그것이 {@code task_resource} 를 만든 이유이고, 새로 만드는 작업은 그쪽에 기록된다.
     */
