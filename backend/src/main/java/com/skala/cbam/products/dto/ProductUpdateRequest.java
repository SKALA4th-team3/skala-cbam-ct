...
public record ProductUpdateRequest(
        ...
        @Size(min = 1, message = "수출 대상 국가는 하나 이상 필요합니다")
        List<@NotBlank @Pattern(regexp = "^[A-Z]{2}$",
                message = "국가 코드는 대문자 2자리여야 합니다") String> exportCountries,

        @Size(min = 1, message = "구성 부품은 하나 이상 필요합니다")
        List<@Valid PartRequest> parts
) {
    public record PartRequest(
            @NotNull(message = "부품 ID는 필수입니다") Long partId,
            @NotNull(message = "협력사 ID는 필수입니다") Long supplierId,
